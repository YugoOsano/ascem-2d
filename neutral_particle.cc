...
// Oxygenの入射から放出、吸着まで
void Neutral_class::
all_process_oxygen(class Shape_trim_class *Shape,
		   class Shape_counter_class *Neutral_counter,
		   bool   flag_inject_from_side ,
		   bool   flag_flux_count ,
		   double incident_energy )

{
  ...
  while(1) 
	 ...
	 [i_cell_x][i_cell_z] != SHAPE_SPACE )
	{
	  //---- 吸着 -----
	  flag_adsorption = 
	    Shape->oxidation_Si(i_cell_x , i_cell_z,  // 吸着した？
				ADSORPTION_PROBABILITY_O) ;//1.0);//
	      if(flag_adsorption == true )
		break;   // 吸着したらループを出る
	      ...
	      //     やり直し(50回になったらもうやめる)
	      i_exception_reflect = 0;
	      Shape->get_surfacenormal(i_cell_x, i_cell_z) ;
	reflection:
	      if( i_exception_reflect > 50 )
		break ;
	      ...
	      random_reflection //(FALSE,0.0,0.0);//乱反射
		(true,//<-cosine diffusion を導入すると左右非対称になる<- this bug was fixed (29Aug2004)
		 Shape->surfacenormal_x[i_cell_x][i_cell_z],
		 Shape->surfacenormal_z[i_cell_x][i_cell_z]) ;
...
/*
{
  ...
  while(1) 
	   ...
	   &i_x, &i_z, &i_x_particle, &i_z_particle ) != SHAPE_SPACE )
	{
	  //---- 吸着 -----
	  flag_adsorption = 
	    Shape->oxidation_Si(i_x , i_z,  // 吸着した？
				ADSORPTION_PROBABILITY_O) ;//1.0);//
	  if(flag_adsorption == true )
	    break;   // 吸着したらループを出る
