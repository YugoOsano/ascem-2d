...
void Particle_class::inject_from_center(double v_z)
{
  ...
  pos_v.x = SYSTEM_WIDTH_X / 2.0 + BEAM_WIDTH * CELL_SIZE * (1.0 - 2.0*RAN0()) ; 
  pos_v.y = RAN0() * CELL_SIZE ;//0.0 ;
  pos_v.z = RAN0() * CELL_SIZE ;//0.0 ;

  if (RANDOM_ROTATION_ANGLE_FLAG == true){
    pos_v.v_r     =  v_z ;
  ...
  v_xx = pos_v.v_r * sin(pos_v.v_psi) * cos(pos_v.v_theta) ;
