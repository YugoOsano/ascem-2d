...
Shape_trim_class::Shape_trim_class(bool flag_open_space)
{
  ...
  for(int i_z = 0; i_z < N_CELL_Z ; i_z++)
      ...
      if ( i_z < N_from_TOP_to_MASK )
	{
      ...
      //MASK
      else if((i_z >= N_from_TOP_to_MASK ) && (i_z < N_from_TOP_to_MASK + N_MASK))
	{ 
	  for(int i_x = 0; i_x < N_CELL_X ; i_x++){
	    //Mask側壁に傾斜角がある場合
	    if( i_x <  N_LEFT_MASK   - ((N_from_TOP_to_MASK + N_MASK - i_z) * tan(SLOPE_ANGLE_SIDEWALL)) ||
		i_x >= N_RIGHT_MASK + ((N_from_TOP_to_MASK + N_MASK - i_z) * tan(SLOPE_ANGLE_SIDEWALL))  ){
	      shape_matrix[i_x][i_z] = HARD_MASK ;
	      
	      //片側 open space の場合==右側のMaskはなし
      ...
      //Si_1
      else if((i_z >= N_from_TOP_to_MASK + N_MASK ) && (i_z < N_from_TOP_to_MASK + N_MASK + N_Si))
	{ 
	  for(int i_x = 0; i_x < N_CELL_X ; i_x++){
	    //Si側壁に傾斜角がある場合
	    if( i_x <  N_LEFT_MASK   + ((i_z - N_from_TOP_to_MASK - N_MASK) * tan(SLOPE_ANGLE_SIDEWALL_Si)) ||
	       i_x >= N_RIGHT_MASK - ((i_z - N_from_TOP_to_MASK - N_MASK) * tan(SLOPE_ANGLE_SIDEWALL_Si))  ){
	      shape_matrix[i_x][i_z] = SHAPE_Si ;
	      
	      //片側 open space の場合==右側のSi側壁はなし
	      if( flag_open_space == true &&
		  i_x >= N_RIGHT_MASK  - ((i_z - N_from_TOP_to_MASK - N_MASK) * tan(SLOPE_ANGLE_SIDEWALL_Si)) ){
		shape_matrix[i_x][i_z] = SHAPE_SPACE ;
	      }
      ...
      //Si_2 
      else if((i_z >= N_from_TOP_to_MASK + N_MASK + N_Si ) && (i_z < N_from_TOP_to_MASK + N_MASK + N_Si + N_Si_2 ))
	{ 
	  for(int i_x = 0; i_x < N_CELL_X ; i_x++){
	    //Si側壁に傾斜角がある場合
	    if( i_x <  N_LEFT_MASK   + N_Si * tan(SLOPE_ANGLE_SIDEWALL_Si) + ((i_z - N_from_TOP_to_MASK - N_MASK - N_Si) * tan(SLOPE_ANGLE_SIDEWALL_Si_2)) ||
		i_x >= N_RIGHT_MASK - N_Si * tan(SLOPE_ANGLE_SIDEWALL_Si) - ((i_z - N_from_TOP_to_MASK - N_MASK - N_Si) * tan(SLOPE_ANGLE_SIDEWALL_Si_2))  ){
	      shape_matrix[i_x][i_z] = SHAPE_Si ;
      ...
      //from Si_2 bottom to SiO2 layer top
      else if(i_z >= N_from_TOP_to_MASK + N_MASK + N_Si + N_Si_2 && i_z < N_from_TOP_to_MASK + N_MASK + N_Si + N_Si_2 + N_from_Si_bottom_to_SiO2_layer_top)
	{ 
	  for(int i_x = 0; i_x < N_CELL_X ; i_x++)
	    shape_matrix[i_x][i_z] = SHAPE_Si;
	}
      ...
      //SiO2 layer
      else if(i_z >= N_from_TOP_to_MASK + N_MASK + N_Si + N_Si_2 + N_from_Si_bottom_to_SiO2_layer_top && i_z < N_from_TOP_to_MASK + N_MASK + N_Si + N_Si_2 + N_from_Si_bottom_to_SiO2_layer_top + N_under_Si_layer)//SiO2layer
	{
	  for(int i_x = 0; i_x < N_CELL_X ; i_x++)
	    shape_matrix[i_x][i_z] = HARD_MASK ;
	}
      ...
      //Under Si layer
      else if(i_z >= N_from_TOP_to_MASK + N_MASK + N_Si + N_Si_2 + N_from_Si_bottom_to_SiO2_layer_top + N_under_Si_layer)//UnderSilayer
	{
	  for(int i_x = 0; i_x < N_CELL_X ; i_x++)
	    shape_matrix[i_x][i_z] = SHAPE_Si ;
	}
  ...
  for(int i_z = 0; i_z < N_CELL_Z ; i_z++)
    {
      for(int i_x = 0; i_x < N_CELL_X ; i_x++)
	{
          if(FLAG_SATURATED_SURFACE == true && FLAG_SATURATED_SURFACE_OXYGEN == false && shape_matrix[i_x][i_z] == SHAPE_Si){
           n_Clbond[i_x][i_z] = 4 ;
	   n_oxygen[i_x][i_z] = 0 ;
	  }

          else if(FLAG_SATURATED_SURFACE_OXYGEN == true && FLAG_SATURATED_SURFACE == false && shape_matrix[i_x][i_z] == SHAPE_Si){
           n_Clbond[i_x][i_z] = 0 ;
	   n_oxygen[i_x][i_z] = 2 ;
	  }

          else if(FLAG_SATURATED_SURFACE_OXYGEN == true && FLAG_SATURATED_SURFACE == true && shape_matrix[i_x][i_z] == SHAPE_Si){
           n_Clbond[i_x][i_z] = 0 ;
	   n_oxygen[i_x][i_z] = 2 ;
	  }
			    ...
			    int *i_x_particle, int *i_z_particle) 
{
  ...
  if(put_shape_matrix(x, z , i_x, i_z ) != SHAPE_SPACE)
      ...
      else 
	{
      ...
      z_bottom  = z - flag_z * CELL_SIZE ;
      z_top     = z + flag_z * CELL_SIZE ;
      
      // x方向、z方向のどちらの成分が大きいか判定し、それに応じた
      // 順に処理をする
	 if(fabs(v_x) >= fabs(v_z))
	{ 
	  if(fabs(v_z)  >= tan( PI / 8.0 ) * fabs(v_x) )
	    {
	  
	      if(put_shape_matrix(x_right, z_top, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x_right, z, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x , z_top, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x_right , z_bottom, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x_left , z_top, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x , z_bottom, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x_left , z, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	      if(put_shape_matrix(x_left , z_bottom, 
			      i_x, i_z ) != SHAPE_SPACE)
		return shape_matrix[*i_x][*i_z];
	    }
	  else
	...
	{
	  if(fabs(v_x)  >= tan( PI / 8.0 ) * fabs(v_z) )
	    {
...
double etch_yield_angle(double theta) 
{
  double tmp_coef = 0.0; 
