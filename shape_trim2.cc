...
void  Shape_trim_class::remove_isolated_Si() 
{
  ...
  for( i_z = N_TOP_MASK; i_z < N_CELL_Z - N_TOP_MASK ; i_z++ )
    {
      for( i_x = 1; i_x < N_CELL_X - 1; i_x++ )
	{
	  ...
	  if(shape_matrix[i_x][i_z] == SHAPE_Si)
		 ...
		 shape_matrix[i_x + 1][i_z + 1] == SHAPE_SPACE )
		{
		 ...
		 shape_matrix[i_x + 10][i_z ] == SHAPE_SPACE &&
		 
		 (shape_matrix[i_x    ][i_z + 1 ] == SHAPE_SPACE ||
		  shape_matrix[i_x    ][i_z + 3 ] == SHAPE_SPACE ||
		  shape_matrix[i_x    ][i_z + 5 ] == SHAPE_SPACE ||
 		  shape_matrix[i_x    ][i_z + 10] == SHAPE_SPACE ))
		{
		  for(int i = 0; i< 10; i++)
		    {		  
		      shape_matrix[i_x + i][i_z] = SHAPE_SPACE ;
		      n_Clbond[i_x + i ][i_z]     = 0 ;
		      n_oxygen[i_x + i ][i_z]     = 0 ;
		    }
		  cout << i_x << "\t" << i_z << "\n" ;
		}
...
void  Shape_trim_class::remove_isolated_Si() 
{
  ...
  for( i_z = N_from_TOP_to_MASK; i_z < N_CELL_Z - N_from_TOP_to_MASK ; i_z++ )
    {
      for( i_x = 1; i_x < N_CELL_X - 1; i_x++ )
	{
	  //-- そのセルはSi 原子があるか？
	  if(shape_matrix[i_x][i_z] == SHAPE_Si)
		 ...
		 shape_matrix[i_x + 1][i_z + 1] == SHAPE_SPACE )
		{
	      ...
	      if(shape_matrix[i_x - 1 ][i_z ] == SHAPE_SPACE &&
		 (shape_matrix[i_x + 1 ][i_z ] == SHAPE_SPACE ||
		  shape_matrix[i_x + 3 ][i_z ] == SHAPE_SPACE ||
		  shape_matrix[i_x + 5 ][i_z ] == SHAPE_SPACE ||
		  shape_matrix[i_x + 10][i_z ] == SHAPE_SPACE) &&
...
Shape_counter_class::Shape_counter_class()
{}
Shape_counter_class::~Shape_counter_class()
{
  delete [] p_ctr ;
...
// -- メモリ確保 & カウンタ初期化 --
void Shape_counter_class::allocate_memory()
{
...
void Shape_counter_class::count(int i_x, int i_z )
{
  ctr[i_x][i_z]++ ;
}
